...
impl WallpaperConfigUI {
    pub fn new(wallpaper_conf: WallpaperConf, wallpaper_items: Vec<WallpaperItem>) -> Self {
        Self {
            selected_wallpaper: wallpaper_items
                .iter()
                .position(|item| item.path == wallpaper_conf.wallpaper_path),
    ...
    pub fn view(&mut self) -> Element<WallpaperConfigMsg, Renderer> {
        ...
        let wallpaper_grid = wallpaper_items.iter_mut().enumerate().fold(
            ...
            |grid, (idx, (state, item))| {
                ...
                let mut btn = Button::new(state, Column::new().spacing(10).push(image).push(name))
                    .padding(7)
