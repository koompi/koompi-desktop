...
use iced_winit::{
    ...
    Command, Container, Element, Grid, Length, PickList, Program, Row, Scrollable, Space, Text,
    TextInput,
};
    ...
    fn new(flags: Self::Flags) -> (Self, Command<BackgroundConfMsg>) {
        (
            Self {
                ...
                wallpaper_items: flags
                    .1
                    .into_iter()
                    .map(|item| (button::State::new(), item))
                    .collect(),
    ...
    fn view(&mut self) -> Element<Self::Message, Renderer> {
        ...
        let content: Element<_, _> = match bg_conf.kind {
            BackgroundType::Color => {
                ...
                let txt_color =
                    TextInput::new(&mut self.color_state, "", &self.text, ColorChanged).padding(7);
                Row::new()
                    .spacing(15)
            ...
            BackgroundType::Wallpaper => {
                ...
                let pl_placement = PickList::new(
                    placement_state,
                    &Placement::ALL[..],
                    Some(bg_conf.wallpaper_conf.placement),
                    PlacementChanged,
                );
                let wallpaper_grid = wallpaper_items.iter_mut().enumerate().fold(
                    Grid::new().column_width(140).padding(15).spacing(15),
                    |grid, (idx, (state, item))| {
                        let name = Text::new(
                            item.name
                                .as_ref()
                        ...
                        let image = Image::new(item.path.to_path_buf())
                            .width(Length::Units(100))
                            .height(Length::Units(60));
                        let mut btn =
                            Button::new(state, Column::new().spacing(10).push(image).push(name))
                                .padding(7)
                                .width(Length::Units(120))
                                .on_press(WallpaperChanged(idx));
                        ...
                        grid.push(Container::new(btn).center_x().center_y())
                    },
                );
