...
use iced_winit::{conversion, futures, winit, program, Debug};
...
fn main() {
   ...
   let window = WindowBuilder::new().with_x11_window_type(vec![XWindowType::Desktop]).build(&event_loop).unwrap();
   ...
   let (adapter, (mut device, queue)) = futures::executor::block_on(async {
      ...
      }).await.expect("Request adapter");

      let dev_queue = adapter.request_device(
      ...
      *control_flow = ControlFlow::Wait;
      ...
      match event {
         ...
         Event::RedrawRequested(_) => {
            ...
            local_pool.spawner().spawn(staging_belt.recall()).expect("Recall staging buffers");
